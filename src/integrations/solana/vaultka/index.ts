...
import marginfiIdl from '../project0/idls/marginfi_0.1.7.json'
import {
...
const VAULTKA_V2_MARGINFI_PROGRAM_ID =
...
export const PROGRAM_IDS = [
  ...PROJECT0_PROGRAM_IDS,
  VAULTKA_V2_MARGINFI_PROGRAM_ID,
] as const
...
const VAULTKA_ALLOWED_GROUPS = new Set([VAULTKA_LEGACY_GROUP, VAULTKA_V2_GROUP])
...
function decodeSignedI128LE(bytes: number[] | Uint8Array): bigint {
  ...
  if ((buf[15] ?? 0) & 0x80) value -= 1n << 128n
  return value
}
...
function pushAggregatedAmount(
  ...
  map.set(mint, { mint, decimals, amountRaw })
}
...
async function* getVaultkaV2Positions(
  address: string,
  { tokens }: SolanaPlugins,
): UserPositionsPlan {
  const marginfiAccountsMap = yield {
    kind: 'getProgramAccounts' as const,
    programId: VAULTKA_V2_MARGINFI_PROGRAM_ID,
    filters: [
      {
        memcmp: {
          offset: 0,
  ...
  for (const [accountAddress, account] of Object.entries(marginfiAccountsMap)) {
    ...
    const activeBalances = decoded.lending_account.balances.filter((balance) => {
  ...
  if (parsedAccounts.length === 0 || bankAddressSet.size === 0) return []
  ...
  for (const parsedAccount of parsedAccounts) {
    const positionsByOrigin = new Map<OriginProtocol, AggregatedPositionByOrigin>()
    ...
    for (const balance of parsedAccount.balances) {
      ...
      if (originProtocol === 'kamino' && suppliedRaw > 0n) {
        ...
        if (reserve) suppliedRaw = collateralToUnderlyingAmount(suppliedRaw, reserve)
      }
      ...
      if (suppliedRaw > 0n) {
        pushAggregatedAmount(aggregated.suppliedByMint, mint, decimals, suppliedRaw)
      }
      if (borrowedRaw > 0n) {
        pushAggregatedAmount(aggregated.borrowedByMint, mint, decimals, borrowedRaw)
      }
    }
...
function remapVaultkaPositions(
  ...
  for (const position of positions) {
    ...
    if (
      typeof typedMeta.group !== 'string' ||
      !VAULTKA_ALLOWED_GROUPS.has(typedMeta.group)
    ) {
      continue
    }
...
export const vaultkaIntegration: SolanaIntegration = {
  ...
  getUserPositions: async function* (
    ...
    const allPositions: UserDefiPosition[] = []

    const getProject0Positions = project0Integration.getUserPositions
    if (getProject0Positions) {
      const legacyPlan = getProject0Positions(address, plugins)
      let step = await legacyPlan.next()
      while (!step.done) {
        const accounts = yield step.value
        step = await legacyPlan.next(accounts)
      }
      allPositions.push(...step.value)
    }

    const v2Plan = getVaultkaV2Positions(address, plugins)
    let v2Step = await v2Plan.next()
    while (!v2Step.done) {
      const accounts = yield v2Step.value
      v2Step = await v2Plan.next(accounts)
    }
    allPositions.push(...v2Step.value)

    return remapVaultkaPositions(allPositions)
  },
}
