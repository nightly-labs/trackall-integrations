...
function decodeKlendObligation(
	...
	const decoded = Obligation.decode(Buffer.from(accountData));
...
function decodeKlendReserve(accountData: Uint8Array): KlendReserveDecoded {
	const decoded = Reserve.decode(Buffer.from(accountData));
...
function decodeKVaultSharesMintAndDecimals(
	...
	const decoded = VaultState.decode(Buffer.from(accountData));
	...
	getUserPositions: async function* (
		...
		for (const account of Object.values(phase1Map)) {
			...
			if (account.programAddress === FARMS_PROGRAM_ID) {
				try {
					const userState = UserState.decode(Buffer.from(account.data));
		...
		const farmsByAddress = new Map<string, FarmState>();
		for (const farmAddress of farmAddresses) {
			...
			try {
				farmsByAddress.set(
					farmAddress,
					FarmState.decode(Buffer.from(account.data)),
				);
			} catch {
